...
ANSI_TO_KEY: Dict[bytes, Union[Key, Tuple[Key, ...]]] = {
    b"\x00": Key.CTRL_AT,  # CTRL_-At (Also for Ctrl-Space)
    ...
    b"\x08": Key.CTRL_H,  # CTRL_-H (8) (Identical to '\b')
    b"\x09": Key.CTRL_I,  # CTRL_-I (9) (Identical to '\t')
    ...
    b"\x0d": Key.CTRL_M,  # CTRL_-M (13) (Identical to '\r')
    ...
    b"\x7f": Key.CTRL_H,  # ASCII Delete (0x7f)
    ...
    b"\x1b[1;3D": (Key.ESC, Key.LEFT),
    b"\x1b[1;3C": (Key.ESC, Key.RIGHT),
    b"\x1b[1;3A": (Key.ESC, Key.UP),
    b"\x1b[1;3B": (Key.ESC, Key.DOWN),

    b"\x1b[1;9A": (Key.ESC, Key.UP),
    b"\x1b[1;9B": (Key.ESC, Key.DOWN),
    b"\x1b[1;9C": (Key.ESC, Key.RIGHT),
    b"\x1b[1;9D": (Key.ESC, Key.LEFT),
...
class KeyPress:
    ...
    keyBytes: bytes
    value: str
    key: Key  # defaults to Key.UNKNOWN if there's no match in ANSI_TO_KEY
